  ...
  <Transition name="fade">
    <div v-if="showModal" class="fixed inset-0 bg-[rgba(0,0,0,0.3)] z-20">
      <div class="flex justify-center items-center h-full w-full">
        <div v-if="isMobile()" class="bg-primary-bg rounded-lg py-5 px-6 max-w-xs md:max-w-3xl relative">
          <img src="@/assets/images/home/home-demo-no-mobile.png" alt="手機版本正在進行開發提示圖" />
          ...
          <button type="submit" class="block w-full py-2.5 bg-primary-orange text-secondary-white rounded-md"
            @click="close">
            我知道了
          </button>
        </div>
        <div v-else class="bg-primary-bg rounded-lg py-5 px-6 max-w-xs md:max-w-3xl relative">
          <img src="@/assets/images/client/demo-logo.png" alt="莉莉貝拉維塔義式餐廳 logo" class="max-w-[75%]" />
          <button type="button" class="absolute top-5 md:top-6 end-5 hover:opacity-90" @click="close">
            <span
          ...
          <div class="flex flex-row gap-x-3 max-w-3xl">
            <router-link v-for="role in roles" :key="role.name" :to="{ path: role.path }"
